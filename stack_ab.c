...
/*TREE TRAVERSAL*/
void stack_alpha_beta(struct position pos, int turn)
{
	char child[MAX_DEPTH][6400];
	...
	struct checkers_t glb_stk[MAX_DEPTH + 1];
	...
	while (ply > 0) {
		...
		if (ply < MAX_DEPTH + 1) {
			...
			if (curr_node.iter < curr_node.pos.nc) {
				...
				printf("Depth - %-2d  Iter - %-2d  best_scr - %7d  alpha - %7d  beta - %7d\n", MAX_DEPTH - ply + 1, curr_node.iter, best_score, curr_node.alpha, curr_node.beta);
